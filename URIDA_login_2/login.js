// URIDA_login_2.js - Fixed Version with Proper Error Handling
...
function redirectBasedOnUser(user) {
    try {
        ...
        // Admin user goes to main home page
        if (username === 'admin') {
        ...
        // Since access was already verified, directly redirect to city page
        // window.location.href = `http://127.0.0.1:5500/templates/${cityName}.html`;
        window.location.href = "http://127.0.0.1:5500/home/home.html";
        console.log(`Redirecting to ${cityName} page`);
...
function logout() {
    ...
    fetch('http://192.168.107.49:8060/api/auth/logout', {
        method: 'POST',
        headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
...
async function testBackendConnection() {
    try {
        const response = await fetch('http://192.168.107.49:8060/api/auth/logout', {
            method: 'POST'
        });
...
async function getUserProfile() {
    try {
        const response = await authenticatedFetch('http://192.168.107.49:8060/api/auth/profile');
        const data = await response.json();
