...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const API_BASE_URL = 'http://192.168.105.177:8060/api/auth';
...
function logout() {
    ...
    fetch('http://192.168.105.177:8060/api/auth/logout', {
        method: 'POST',
        headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
...
async function testBackendConnection() {
    try {
        const response = await fetch('http://192.168.105.177:8060/api/auth/logout', {
            method: 'POST'
        });
...
async function getUserProfile() {
    try {
        const response = await authenticatedFetch('http://192.168.105.177:8060/api/auth/profile');
        const data = await response.json();
