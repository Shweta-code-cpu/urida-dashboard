...
let allFetchedData = [];
let vectorLayer_Roads = null; // Declare globally if needed to reuse/remove later
...
function fetchRoadsGreaterThan5() {
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            ...
            if (Array.isArray(data)) {
                allFetchedData = allFetchedData.concat(data);
                populateTableWithData(allFetchedData);
                renderRoadsOnMap(data); // 🔍 Draw roads on the map
            } else {
                console.error('Invalid data structure:', data);
            }
        })
...
function renderRoadsOnMap(roadsData) {
    ...
    roadsData.forEach(item => {
        ...
        if (wkt && wkt.startsWith("MULTILINESTRING")) {
            try {
                const feature = format.readFeature(wkt, {
                    dataProjection: 'EPSG:4326',
                    featureProjection: 'EPSG:3857'  // or match your map.getView().getProjection()
                });
    ...
    if (extent && extent[0] !== Infinity) {
        ...
        alert("Extent is invalid. Features may not have proper coordinates.");
    }
}


// Function to fetch all roads with YoC between 1 and 5 when "Year between 1 to 5" tab is clicked
function fetchRoadsBetween1And5() {
    const apiUrl = `${BASE_URL}/filter-roads-between-1-and-5`;  
...
// Event listener for "Greater than 5" tab click
document.getElementById('tab1').addEventListener('click', function() {
...
function filterRoads() {
    ...
    // Filter the allRoads based on the selected conditions and amenities
    const filteredRoads = allFetchedData.filter(road => {
        ...
        return conditionMatch && amenityMatch;  // Return true if both conditions and amenities match
    });
...
document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
    checkbox.addEventListener('change', function() {
        filterRoads();  // Filter roads when any checkbox is checked/unchecked
    });
});


// Function to populate the table with data (to avoid redundancy)
