//const BASE_URL = "http://192.168.100.178:8080/Ghaziabad/Dashboard/Ghaziabad";
const BASE_URL = `${BASE_URL_All}:8983/Ghaziabad`;
const GEOSERVER_BASE_URL = "http://localhost:8080/geoserver/GZB_Summary";
...
var view = new ol.View({
    ...
    center: [77.3856945136889, 28.66993073148757],
    zoom: 12.5,
});
...
// document.getElementById("zoom-tool-icon").classList.toggle("zoom-active", zoomToolActive);

// document.getElementById("zoom-tool-icon").addEventListener("click", function () {
//     zoomToolActive = !zoomToolActive;
//     if (zoomToolActive) {
//         map.getViewport().style.cursor = 'zoom-in';
//         alert("Zoom Tool Activated: Click on a road to zoom in");
//     } else {
...
function data_analysis() {
    window.open('https://lookerstudio.google.com/reporting/0339b677-47c8-4f4f-b6f0-4daed0e60333/page/p_o8dmprt7bd', '_blank'); // Open Google in a new tab
}
...
//     const sidebar = document.getElementById('sidebar_chart');
...
// Define the color map for road conditions
...
// Define the color map for road ownership types
const ownershipColors = {
    "Anaaj Mandi": "red",
    "Ghaziabad Development Authority": "orange",
...
// Define the color map for road material types
const materialColors = {
    "Bitumin": "#3e3e3e",        // Dark Gray
    "CC": "#8080ff",             // Light Blue
    "Interlocking Road": "#ff9933", // Orange
    "Kacchi Road": "#996633",    // Brown
    "Proposed Road": "#aaaaaa"   // Light Gray
};

// Declare the global chart variables
let chartCondition, chartOwnership, chartMaterial;

/// Function to fetch chart data for a specific zone and update the charts
function fetchChartData(zoneNo) {
    fetch(`${BASE_URL}/getChartDataByZone1?zone_no=${zoneNo}`)
        ...
        .then(data => {
            if (data.status && data.data) {
                ...
                updateChart(chartCondition, 'chart-condition', 'Road Condition', chartData.conditionData, conditionColors);
                updateChart(chartOwnership, 'chart-ownership', 'Ownership', chartData.ownershipData, ownershipColors);
                updateChart(chartMaterial, 'chart-material', 'Material', chartData.materialData, materialColors);
            } else {
                console.warn('No data found for the selected zone.');
            }
        })
        .catch(error => {
...
// Function to update a chart (pie chart in this case)
// Function to update a chart (pie chart in this case)
function updateChart(canvasId, label, data, colorMap) {
    ...
    const labels = data.map(item => item.condition || item.ownership || item.carriageM || 'Unknown');
    const values = data.map(item => item.count);

    // Generate colors for each slice using the colorMap
    const colors = labels.map(label => colorMap[label] || '#cccccc');  // Default to gray if no color found
...
//   fetch(`${BASE_URL}/getConditionStats`, {
//     method: 'POST'
//   })
//   .then(response => response.json())
//   .then(res => {
//     if (!res.status) {
//       alert("Failed to load condition stats");
//       return;
//     }
...
//     new Chart(ctx, {
//       type: 'bar', // you can use 'pie', 'doughnut', etc.
//       data: {
//         labels: labels,
//         datasets: [{
//           label: 'Road Condition Count',
//           data: values,
...
//           title: {
//             display: true,
//             text: 'Road Condition Statistics (Good / Moderate / Poor)'
//           },
//           legend: {
//             display: false
//           }
...
//     alert("Chart failed to load.");
//   });
// });
//-----------------------------------Boundaries------------------------------------//
var zone_boundary = new ol.layer.Image({
    title: "Ghaziabad Zone Boundary",
    //  extent: [-180, -90, -180, 90],
    source: new ol.source.ImageWMS({
        url: `${GEOSERVER_BASE_URL}/wms?`,
        params: {
            LAYERS: "GZB_Summary:ghaziabad_zone_boundary",
        },
        ratio: 1,
...
var ward_boundary = new ol.layer.Image({
    title: "Ghaziabad Ward Boundary",
    //  extent: [-180, -90, -180, 90],
    source: new ol.source.ImageWMS({
        url: `${GEOSERVER_BASE_URL}/wms?`,
        params: {
            LAYERS: "GZB_Summary:ghaziabad_ward_boundary",
        },
        ratio: 1,
        serverType: "geoserver",
    }),
...
function amenitiesRoadAnalysis(endpoint, layerName, featureFunction, elementId) {
    ...
    map.getLayers().getArray().slice().forEach(layer => {
        if (layer instanceof ol.layer.Vector && layer != parkVectorLayer) {            //&& layer != SlumVectorLayer
            map.removeLayer(layer);
        }
    });
    map.getOverlays().clear();
...
Bank_Road.addEventListener('click', () => showAmenityWithRoads('roads-with-atms', 'Bank with Roads', addMultilinestringFeatureFromWKT, 'bank', './logo/bus/bank.png', 'showBanks'));
...
Hospital_Road.addEventListener('click', () => showAmenityWithRoads('roads_with_hospital', 'Hospital with Roads', addMultilinestringFeatureFromWKT_HospitalRoad, 'hospital', './logo/bus/hospital.png', 'showHospital'));
Hotel_Road.addEventListener('click', () => showAmenityWithRoads('roads-with-hotel', 'Hotel with Roads', addMultilinestringFeatureFromWKT_HotelRoad, 'hotel', './logo/bus/hotel.png', 'showHotel'));
Education_Road.addEventListener('click', () => showAmenityWithRoads('roads-with-Education', 'Educational Institute with Roads', addMultilinestringFeatureFromWKT_EduRoad, 'education', './logo/bus/education.png', 'showEducation'));
...
function clearAllVectorLayers() {
    ...
    map.getLayers().getArray().slice().forEach(layer => {
        if ((layer instanceof ol.layer.Tile || layer instanceof ol.layer.Image) &&
            ...
            if (layer.get('title') !== 'Ghaziabad Zone Boundary' && layer.get('title') !== 'Ghaziabad Ward Boundary') {
...
function zoomToIndia() {
    ...
    view.setCenter([77.3856945136889, 28.66993073148757]);
    view.setZoom(12);
    view.setRotation(0);
...
//             url: `${GEOSERVER_BASE_URL}/wms`,
//             params: {
//                 'LAYERS': 'GZB_Summary:annviewpoints',
//             },
//             ratio: 1,
//             serverType: 'geoserver'
//         })
...
const data1 = {
    'Summary': {},
    'Zone 1': { wards: ['Ward 9', 'Ward 16', 'Ward 18', 'Ward 22', 'Ward 24', 'Ward 30', 'Ward 42', 'Ward 46', 'Ward 47', 'Ward 53', 'Ward 56', 'Ward 62', 'Ward 65', 'Ward 67', 'Ward 69', 'Ward 71', 'Ward 84', 'Ward 91'] },
    'Zone 2': { wards: ['Ward 6', 'Ward 8', 'Ward 9', 'Ward 11',  'Ward 12', 'Ward 13', 'Ward 17', 'Ward 19', 'Ward 22','Ward 31', 'Ward 32', 'Ward 33', 'Ward 39', 'Ward 45', 'Ward 49', 'Ward 50', 'Ward 52', 'Ward 59', 'Ward 88', 'Ward 92','Ward 93'] },
    'Zone 3': { wards: ['Ward 1', 'Ward 2', 'Ward 3', 'Ward 4', 'Ward 7', 'Ward 14', 'Ward 15', 'Ward 23', 'Ward 25', 'Ward 26', 'Ward 27', 'Ward 35', 'Ward 48', 'Ward 51', 'Ward 55', 'Ward 58' ] },
    'Zone 4': { wards: ['Ward 5', 'Ward 10', 'Ward 18', 'Ward 28', 'Ward 29', 'Ward 34', 'Ward 37', 'Ward 38', 'Ward 29', 'Ward 34', 'Ward 37', 'Ward 38', 'Ward 44', 'Ward 45', 'Ward 60', 'Ward 63', 'Ward 64', 'Ward 66', 'Ward 70', 'Ward 73', 'Ward 75', 'Ward 78',, 'Ward 80', 'Ward 82', 'Ward 83', 'Ward 85', 'Ward 86', 'Ward 90', 'Ward 92'] },
     'Zone 5': {wards: ['Ward 19', 'Ward 21', 'Ward 36', 'Ward 40', 'Ward 41', 'Ward 43', 'Ward 54', 'Ward 57', 'Ward 61', 'Ward 68', 'Ward 72', 'Ward 74', 'Ward 76', 'Ward 77','Ward 79', 'Ward 81',, 'Ward 87', 'Ward 89', 'Ward 94', 'Ward 97', 'Ward 98', 'Ward 99']},
...
function navigateTo(tabName) {
    ...
    if (tabName === 'Summary') {
        ...
        if (selectedZone) {
          ...
          //   loadConditionChart(selectedZone, null);  // 👈 chart updates for zone
        } else {
        ...
        if (selectedZone) {
            ...
            // loadConditionChart(selectedZone, selectedWard); // 👈 chart updates for zone + ward
        } else {
...
function updateSummary(data) {
    ...
    const summaryData = {
        'No. of Zones': { value: safeValue('zone_count'), onclick: 'Ghaziabad_Zone_no()' },
        'Total Road Length': { value: `${safeValue('total_length_km')} km`, onclick: "ghaziabad_road_Length_Count(); updateNavBarWithFunctionName('Total Road Length');" },
        'Total No. of Roads': { value: safeValue('total_length_count'), onclick: "ghaziabad_road_Length_Count(); updateNavBarWithFunctionName('Total Road Count');" },
        'Total Ward No.': { value: safeValue('ayo_ward'), onclick: 'Ghaziabad_Ward_NO()' },
        'Road Count by Condition': {
            value: `
                Good - <a href="javascript:void(0)" onclick="Ghaziabad_Condition_cat('condition','Good'); updateNavBarWithFunctionName('Road Condition Good');" style="color:green;">${safeValue('condition_count_green')}</a><br>
                Poor - <a href="javascript:void(0)" onclick="Ghaziabad_Condition_cat('condition','Poor'); updateNavBarWithFunctionName('Road Condition Poor');" style="color:red;">${safeValue('condition_count_red')}</a><br>
                Moderate - <a href="javascript:void(0)" onclick="Ghaziabad_Condition_cat('condition','Moderate'); updateNavBarWithFunctionName('Road Condition Moderate');" style="color:yellow;">${safeValue('condition_count_yellow')}</a><br>
              NA - <a href="javascript:void(0)" onclick="Ghaziabad_Condition_cat('condition','NA'); updateNavBarWithFunctionName('Road Condition NA');" style="color:pink;">${safeValue('condition_count_na')}</a><br>
            `
        },
        // 'Road Count by Type': {
        //     value: `
        //         Major - <a href="javascript:void(0)" onclick="Ghaziabad_Type_cat('type','Major City Road'); updateNavBarWithFunctionName('Road Type Major');" style="color:blue;">${safeValue('count_major')}</a><br>
        //         Minor - <a href="javascript:void(0)" onclick="Ghaziabad_Type_cat('type','Minor City Road'); updateNavBarWithFunctionName('Road Type Minor');" style="color:yellow;">${safeValue('count_minor')}</a>
        //     `,
        //     onclick: 'Ghaziabad_Types()'
        // },
        'Road Count by Material': {
            value: `
                Bitumen - <a href="javascript:void(0)" onclick="Ghaziabad_Material_cat('material','Bitumen'); updateNavBarWithFunctionName('Road Material Bitumen');" style="color:darkred;">${safeValue('count_bitumen')}</a><br>
                CC - <a href="javascript:void(0)" onclick="Ghaziabad_Material_cat('material','CC'); updateNavBarWithFunctionName('Road Material CC');" style="color:#1ad7b0;">${safeValue('count_cc')}</a><br>
                Interlocking - <a href="javascript:void(0)" onclick="Ghaziabad_Material_cat('material','Interlocking'); updateNavBarWithFunctionName('Road Material Interlocking');" style="color:#2392ed;">${safeValue('count_interlocking')}</a><br>
                Kachcha - <a href="javascript:void(0)" onclick="Ghaziabad_Material_cat('material','Kachcha'); updateNavBarWithFunctionName('Road Material Kachcha');" style="color:#6036d0;">${safeValue('count_kachcha')}</a><br>
                NA - <a href="javascript:void(0)" onclick="Ghaziabad_Material_cat('material','NA'); updateNavBarWithFunctionName('Road Material NA');" style="color:#dfc223;">${safeValue('count_na')}</a>
                `,
            onclick: 'Ghaziabad_Material()'
        },
        'Road Count by Ownership': {
            value: `
                GNN - <a href="javascript:void(0)" onclick="Ghaziabad_Ownership_cat('ownership','GNN'); updateNavBarWithFunctionName('Road Ownership Ghaziabad Nagar Nigam');" style="color: #5aeee5;">${safeValue('count_nn')}</a><br>
                PWD - <a href="javascript:void(0)" onclick="Ghaziabad_Ownership_cat('ownership','PWD'); updateNavBarWithFunctionName('Road Ownership PWD');" style="color: #69e70f;">${safeValue('count_pwd')}</a><br>
                Development Authority - <a href="javascript:void(0)" onclick="Ghaziabad_Ownership_cat('ownership','Development Authority'); updateNavBarWithFunctionName('Road Ownership Development Authority');" style="color: #f16a16;">${safeValue('count_department')}</a><br>
                Private Roads - <a href="javascript:void(0)" onclick="Ghaziabad_Ownership_cat('ownership','Private Roads'); updateNavBarWithFunctionName('Road Ownership Private Road');" style="color: #ed2323;">${safeValue('count_pvt')}</a><br>
                Expressway - <a href="javascript:void(0)" onclick="Ghaziabad_Ownership_cat('ownership','Expressway'); updateNavBarWithFunctionName('Road Ownership Expressway');" style="color: #0f92e7;">${safeValue('count_expressway')}</a><br>
                NHAI - <a href="javascript:void(0)" onclick="Ghaziabad_Ownership_cat('ownership','NHAI'); updateNavBarWithFunctionName('Road Ownership NHAI');" style="color: #335bee;">${safeValue('count_nhai')}</a><br>
                Railway - <a href="javascript:void(0)" onclick="Ghaziabad_Ownership_cat('ownership','Railway'); updateNavBarWithFunctionName('Road Ownership Railway');" style="color: yellow;">${safeValue('count_railway')}</a><br>
                Defence - <a href="javascript:void(0)" onclick="Ghaziabad_Ownership_cat('ownership','Defence'); updateNavBarWithFunctionName('Road Ownership Defence');" style="color: #d8b219;">${safeValue('count_defence')}</a><br>
                Others - <a href="javascript:void(0)" onclick="Ghaziabad_Ownership_cat('ownership','Others'); updateNavBarWithFunctionName('Road Ownership Others');" style="color: #831042;">${safeValue('count_others')}</a><br>
                Departmental Road - <a href="javascript:void(0)" onclick="Ghaziabad_Ownership_cat('ownership','Departmental Roads'); updateNavBarWithFunctionName('Road Ownership Departmental Roads');" style="color: #8d139b;">${safeValue('count_department')}</a><br>

         
                `,
            onclick: 'Ghaziabad_Ownership()'
        },
        // 'Road Count by CUS': {
        //     value: `
        //         14th Finance - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','14th Finance'); updateNavBarWithFunctionName('Road CUS 14th Finance');" style="color: #ec1248;">${safeValue('count_14th')}</a><br>
        //         15th Finance - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','15th Finance'); updateNavBarWithFunctionName('Road CUS 15th Finance');" style="color: #e63dee;">${safeValue('count_15th')}</a><br>
        //         Nagar Nigam Nidhi - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','Nagar Nigam Nidhi'); updateNavBarWithFunctionName('Road CUS Nagar Nigam Nidhi');" style="color: cyan;">${safeValue('count_nnn')}</a><br>
        //         PWD - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','PWD'); updateNavBarWithFunctionName('Road CUS PWD');" style="color: #173dd6;">${safeValue('count_cus_pwd')}</a><br>
        //         SFC - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','SFC'); updateNavBarWithFunctionName('Road CUS SFC');" style="color: #14ea54;">${safeValue('count_cus_sfc')}</a><br>
        //         Vidhayak Nidhi - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','Vidhayak Nidhi'); updateNavBarWithFunctionName('Road CUS Vidhayak Nidhi');" style="color: #6e52e7;">${safeValue('count_cus_vidhayak')}</a><br>
        //         Sansad Nidhi - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','Sansad Nidhi'); updateNavBarWithFunctionName('Road CUS Sansad Nidhi');" style="color: #c70ed8;">${safeValue('count_cus_sansad')}</a><br>
        //         Duda - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','Duda'); updateNavBarWithFunctionName('Road CUS Duda');" style="color: #d4b85a;">${safeValue('count_cus_duda')}</a><br>
        //         GN City - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','GN City'); updateNavBarWithFunctionName('Road CUS GN City');" style="color: #e14a22;">${safeValue('count_cus_gncity')}</a><br>
        //         RES - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','RES'); updateNavBarWithFunctionName('Road CUS RES');" style="color: #e6497d;">${safeValue('count_cus_res')}</a><br>
        //         Twarit Yojna - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','TWARIT YOJANA'); updateNavBarWithFunctionName('Road CUS TWARIT YOJANA');" style="color: #1cdad1;">${safeValue('count_cus_twarit')}</a><br>
        //         Ghaziabad Smart City - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','Ghaziabad Smart city'); updateNavBarWithFunctionName('Road CUS Ghaziabad Smart City');" style="color: #f4ee40;">${safeValue('count_cus_Ghaziabadsmartcity')}</a><br>
        //         NAGAR PANCHAYAT NIDHI - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','NAGAR PANCHAYAT NIDHI'); updateNavBarWithFunctionName('Road CUS NAGAR PANCHAYAT NIDHI');" style="color: #bad057;">${safeValue('count_cus_panchayat')}</a><br>
        //         UP Bridge Corporation Ltd - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','Up Bridge Corporation Ltd'); updateNavBarWithFunctionName('Road CUS UP Bridge Corporation Ltd');" style="color: #45d1a5;">${safeValue('count_cus_bridge')}</a><br>

        //         NA - <a href="javascript:void(0)" onclick="Ghaziabad_CUS_cat('cus','NA'); updateNavBarWithFunctionName('Road CUS NA');" style="color: #8717e2;">${safeValue('count_cus_na')}</a><br>


        //         `
        //     ,
        //     // onclick: 'Ghaziabad_CUS()'
        // },
        'Road Count by Category': {
            value: `
                Local Street - <a href="javascript:void(0)" onclick="Ghaziabad_TypeSub_cat('category','Local Street'); updateNavBarWithFunctionName('Road Category Local Street');" style="color: #14cee3;">${safeValue('count_local_street')}</a><br>
                Collector - <a href="javascript:void(0)" onclick="Ghaziabad_TypeSub_cat('category','Collector'); updateNavBarWithFunctionName('Road Category Collector');" style="color: #e63dee;">${safeValue('count_collector')}</a><br>
                Expressway - <a href="javascript:void(0)" onclick="Ghaziabad_TypeSub_cat('category','Expressway'); updateNavBarWithFunctionName('Road Category Expressway');" style="color: #ec1248;">${safeValue('count_expressway')}</a><br>
                Arterial - <a href="javascript:void(0)" onclick="Ghaziabad_TypeSub_cat('category','Arterial'); updateNavBarWithFunctionName('Road Category Arterial');" style="color: #e1ca4c;">${safeValue('count_arterial')}</a><br>
                Sub Arterial - <a href="javascript:void(0)" onclick="Ghaziabad_TypeSub_cat('category','Subarterial'); updateNavBarWithFunctionName('Road Category Subarterial');" style="color: #83e45c;">${safeValue('count_subarterial')}</a><br>
                NA - <a href="javascript:void(0)" onclick="Ghaziabad_TypeSub_cat('category','NA'); updateNavBarWithFunctionName('Road Category NA');" style="color: #ec1248;">${safeValue('count_na')}</a><br>
    ...
    const zoneCards = [
        {
            ...
            value: `<a href="javascript:void(0)" onclick="Ghaziabad_Zone_no('zone_no','${zoneNo}'); updateNavBarWithFunctionName('Zone-${zoneNo} Total Road Length');" style="color:black;">${data.length_km.toFixed(2)} km</a>`
        },
        {
            title: 'Total No. of Roads',
            value: `<a href="javascript:void(0)" onclick="Ghaziabad_Zone_no('zone_no','${zoneNo}'); updateNavBarWithFunctionName('Zone-${zoneNo} Total Road Count');" style="color:black;">${data.total_no_of_roads}</a>`
        },
        // { 
        //     title: 'Road Type', 
        //     value: `Major <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Type('${zoneNo}','type','Major City Road'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Type Major');" style="color:blue;">- ${data.total_major_sum}</a> <br> 
        //             Minor <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Type('${zoneNo}','type','Minor City Road'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Type Minor');" style="color:yellow;">- ${data.total_minor_sum}</a>`
        // },
        {
            title: 'Road Condition',
            value: `Good <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Condition('${zoneNo}','condition','Good'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Condition Good');" style="color:green;">- ${data.count_green}</a> <br> 
                    Moderate <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Condition('${zoneNo}','condition','Moderate'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Condition Moderate');" style="color:yellow;">- ${data.count_yellow}</a> <br> 
                    Poor <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Condition('${zoneNo}','condition','Poor'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Condition Poor');" style="color:red;">- ${data.count_red}</a><br>
                    NA <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Condition('${zoneNo}','condition','NA'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Condition NA');" style="color:pink;">- ${data.count_na}</a> <br> 

                    `
        },
        {
            title: 'Materials',
            value: `Bitumen <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Material('${zoneNo}','material','Bitumen'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material Bitumen');" style="color:darkred;">- ${data.bitumen}</a> <br>
                    CC <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Material('${zoneNo}','material','CC'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material CC');" style="color:#1ad7b0;">- ${data.cc}</a> <br>
                    Interlocking <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Material('${zoneNo}','material','Interlocking'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material Interlocking');" style="color:#2392ed;">- ${data.interlocking}</a> <br>
                    Kachcha <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Material('${zoneNo}','material','Kachcha'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material Kachcha');" style="color:#6036d0;">- ${data.kachcha}</a><br>
                    NA <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Material('${zoneNo}','material','NA'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material NA');" style="color:#1ad7b0;">- ${data.total_na}</a> <br>
`
                },
        {
            title: 'Ownership',
            value: `GNN <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Ownership('${zoneNo}','ownership','GNN'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership Ghaziabad Nagar Nigam');" style="color: #5aeee5;">- ${data.nn}</a> <br>
                    PWD <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Ownership('${zoneNo}','ownership','PWD'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership PWD');" style="color: #69e70f;">- ${data.pwd}</a> <br>
                    Private Roads <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Ownership('${zoneNo}','ownership','Private Roads'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership Private Roads');" style="color: #ed2323;">- ${data.pvt}</a> <br>
                    Development Authority - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Ownership('${zoneNo}', 'ownership', 'Development Authority'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership Ghaziabad Development Authority');" style="color: #f16a16;">${data.department}</a><br>
                    NHAI <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Ownership('${zoneNo}','ownership','NHAI'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership NHAI');" style="color: #335bee;">- ${data.nhai}</a> <br>
                    Expressway <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Ownership('${zoneNo}','ownership','Expressway'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership Expressway');" style="color: #0f92e7;">- ${data.expressway}</a> <br>
                    Railway <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Ownership('${zoneNo}','ownership','Railway'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership Railway');" style="color: yellow;">- ${data.railway}</a> <br>
                    Defence <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Ownership('${zoneNo}','ownership','Defence'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership Defence');" style="color: #d8b219;">- ${data.defence}</a> <br>
                    Departmental Roads <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Ownership('${zoneNo}','ownership','Departmental Roads'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership Departmental Roads');" style="color: #8d139b;">- ${data.department}</a> <br>
                    Others <a href="javascript:void(0)" onclick="Ghaziabad_Zone_Ownership('${zoneNo}','ownership','Others'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership Others');" style="color: #831042;">- ${data.others}</a> <br>

                    `
        },
        // {
        //     title: 'CUS',
        //     value: ` 14th Finance - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','14th Finance'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS 14th Finance');" style="color: #ec1248;">${data.count_finance14th}</a><br>
        //                 15th Finance - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','15th Finance'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS 15th Finance');" style="color: #e63dee;">${data.count_finance15th}</a><br>
        //                 Nagar Nigam Nidhi - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','Nagar Nigam Nidhi'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS Nagar Nigam Nidhi');" style="color:cyan;">${data.count_nnn1}</a><br>
        //                 PWD - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','PWD'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS PWD');" style="color: #173dd6;">${data.count_pwd}</a><br>
        //                 SFC - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','SFC'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS SFC');" style="color: #14ea54;">${data.count_sfc}</a><br>
        //                 Vidhayak Nidhi- <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','Vidhayak'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS Vidhayak');" style="color: #6e52e7;">${data.count_vidhayak}</a><br>
        //                 Sansad Nidhi - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','Sansad Nidhi'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS Sansad Nidhi');" style="color: #c70ed8;">${data.count_sansad}</a><br>
        //                 DUDA - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','Duda'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS DUDA');" style="color: #d4b85a;">${data.count_duda}</a><br>
        //                 GN City - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','GN City'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS GN City');" style="color: #e14a22;">${data.count_gncity}</a><br>
        //                 RES - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','RES'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS RES');" style="color: #e6497d;">${data.count_res}</a><br>
        //                 TWARIT YOJANA - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','TWARIT YOJANA'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS TWARIT YOJANA');" style="color: #1cdad1;">${data.count_twarit}</a><br>
        //                 Ghaziabad Smart City - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','Ghaziabad Smart City'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS Ghaziabad Smart City');" style="color: #e1ca4c;">${data.count_Ghaziabadsmartcity}</a><br>
        //                 Nagar Panchayat Nidhi - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','NAGAR PANCHAYAT NIDHI'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS Nagar Panchayat Nidhi');" style="color: #f4ee40;">${data.count_panchayat}</a><br>
        //                 UP Bridge Corporation Ltd - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','Up Bridge Corporation Ltd'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS UP Bridge Corporation Ltd');" style="color: #45d1a5;">${data.count_bridge}</a><br>
 
        //                 NA - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','NA'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS NA');" style="color: #8717e2;">${data.count_na}</a><br>
        ...
        //                 `  },
        {
            ...
            value: `
                    Local Street - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_TypeSub('${zoneNo}','category','Local Street'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Category Local Street');" style="color: #14cee3;">${data.count_local_street} </a><br>
                    Collector - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_TypeSub('${zoneNo}','category','Collector'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Category Collector');" style="color: #e63dee;">${data.count_collector}</a><br>
                    NA - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_TypeSub('${zoneNo}','category','NA'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Category NA');" style="color: #ec1248;">${data.count_expressway}</a><br>
                    Arterial - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_TypeSub('${zoneNo}','category','Arterial'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Category Arterial');" style="color: #e1ca4c;">${data.count_arterial}</a><br>
                    Sub Arterial - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_TypeSub('${zoneNo}','category','Subarterial'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Category Subarterial');" style="color: #83e45c;">${data.count_subarterial}</a><br>
        ...
        .then(responseData => {
            if (responseData.status) {
                ...
                const details = [
                    // { 
                    //     title: 'Road Type', 
                    //     value: `Major - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Type('${wardNo}', 'type', 'Major City Road'); updateNavBarWithFunctionName('Ward-${wardNo} Road Type Major');" style="color:blue;">${responseData.total_major_sum}</a><br> 
                    //              Minor - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Type('${wardNo}', 'type', 'Minor City Road'); updateNavBarWithFunctionName('Ward-${wardNo} Road Type Minor');" style="color:orange;">${responseData.total_minor_sum}</a>` 
                    // },
                    {
                        title: 'Total Road Length',
                        value: `<a href="javascript:void(0)" onclick="Ghaziabad_Zone_no('ward_no', '${wardNo}'); updateNavBarWithFunctionName('Ward-${wardNo} Total Road Length');" style="color:black;">${responseData.length_km} km</a>`
                    },
                    {
                        title: 'Total No. of Roads',
                        value: `<a href="javascript:void(0)" onclick="Ghaziabad_Zone_no('ward_no', '${wardNo}'); updateNavBarWithFunctionName('Ward-${wardNo} Total Road Count');" style="color:black;">${responseData.total_no_of_roads}</a>`
                    },
                    {
                        title: 'Road Condition',
                        value: `Good - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Condition('${wardNo}', 'condition', 'Good'); updateNavBarWithFunctionName('Ward-${wardNo} Road Condition Good');" style="color:green;">${responseData.count_green}</a> 
                                <br> Moderate - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Condition('${wardNo}', 'condition', 'Moderate'); updateNavBarWithFunctionName('Ward-${wardNo} Road Condition Moderate');" style="color:yellow;">${responseData.count_yellow}</a> 
                                <br> Poor - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Condition('${wardNo}', 'condition', 'Poor'); updateNavBarWithFunctionName('Ward-${wardNo} Road Condition Poor');" style="color:red;">${responseData.count_red}</a>
                                <br> NA - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Condition('${wardNo}', 'condition', 'NA'); updateNavBarWithFunctionName('Ward-${wardNo} Road Condition NA');" style="color:pink;">${responseData.count_na}</a>`

                    },
                    {
                        title: 'Materials',
                        value: `Bitumen - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Material('${wardNo}', 'material', 'Bitumen'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material Bitumen');" style="color:darkred;">${responseData.bitumen}</a> 
                                <br> CC - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Material('${wardNo}', 'material', 'CC'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material CC');" style="color:cyan;">${responseData.cc}</a> 
                                <br> Interlocking - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Material('${wardNo}', 'material', 'Interlocking'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material Interlocking');" style="color:blue;">${responseData.interlocking}</a> 
                                <br> Kachcha - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Material('${wardNo}', 'material', 'Kachcha'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material Kachcha');" style="color:purple;">${responseData.kachcha}</a>
                                <br> NA - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Material('${wardNo}', 'material', 'NA'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material NA');" style="color:pink;">${responseData.na}</a> 
                    ...
                    {
                        title: 'Ownership',
                        value: `GNN - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Ownership('${wardNo}', 'ownership','GNN'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership Ghaziabad Nagar Nigam');" style="color: #5aeee5;">${responseData.nn}</a> 
                                <br> PWD - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Ownership('${wardNo}', 'ownership', 'PWD'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership PWD');" style="color: #69e70f;">${responseData.pwd}</a> 
                                <br> PVT - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Ownership('${wardNo}', 'ownership', 'Private Roads'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership Private Roads');" style="color: #ed2323;">${responseData.pvt}</a> 
                                <br> NHAI - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Ownership('${wardNo}', 'ownership','NHAI'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership National Highways Authority of India');" style="color: #335bee;">${responseData.nhai}</a> 
                                <br> Development Authority - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Ownership('${wardNo}', 'ownership', 'Development Authority'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership Ghaziabad Development Authority');" style="color: #f16a16;">${responseData.da}</a>
                                <br> Railway - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Ownership('${wardNo}', 'ownership', 'Railway'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership Railway');" style="color:yellow;">${responseData.railway}</a> 
                                <br> Defence - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Ownership('${wardNo}', 'ownership', 'Defence'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership Defence');" style="color: #ecc82e;">${responseData.defence}</a> 
                                <br> Departmental Roads - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Ownership('${wardNo}', 'ownership', 'Departmenmtal Roads'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership Departmental Road');" style="color: #8d139b;">${responseData.department}</a>
                                <br> Expressway- <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Ownership('${wardNo}', 'ownership', 'Expressway'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership Expressway');" style="color: #168de7;">${responseData.expressway}</a>
                                <br> Others - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_Ownership('${wardNo}', 'ownership', 'Others'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership Others');" style="color: #831042;">${responseData.others}</a>`


                    },
                    // {
                    //     title: 'CUS',
                    //     value: ` 14th Finance - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_CUS('${wardNo}','cus','14th Finance'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS 14th Finance');" style="color: #ec1248;">${responseData.finance14th}</a><br>
                    //                 15th Finance - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_CUS('${wardNo}','cus','15th Finance'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS 15th Finance');" style="color: #e63dee;">${responseData.finance15th}</a><br>
                    //                 Nagar Nigam Nidhi - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_CUS('${wardNo}','cus','Nagar Nigam Nidhi'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS Nagar Nigam Nidhi');" style="color:cyan;">${responseData.nnn}</a><br>
                    //                 PWD - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_CUS('${wardNo}','cus','PWD'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS PWD');" style="color: #14ea54;">${responseData.sfc}</a><br>
                    //                 SFC - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_CUS('${wardNo}','cus','SFC'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS SFC');" style="color: #173dd6;">${responseData.pwd}</a><br>
                    //                 Vidhayak Nidhi - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_CUS('${wardNo}','cus','Vidhayak Nidhi'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS Vidhayak Nidhi');" style="color: #6e52e7;">${responseData.vidhayak}</a><br>
                    //                 Sansad Nidhi - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_CUS('${wardNo}','cus','Sansad Nidhi'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS Sansad Nidhi');" style="color: #c70ed8;">${responseData.sansad}</a><br>
                    //                 DUDA - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_CUS('${wardNo}','cus','Duda'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS DUDA');" style="color: #d4b85a;">${responseData.duda}</a><br>
                    //                 GN City - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_CUS('${wardNo}','cus','GN City'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS GN City');" style="color: #e14a22;">${responseData.gncity}</a><br>
                    //                 RES - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_CUS('${wardNo}','cus','RES'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS RES');" style="color: #e6497d;">${responseData.res}</a><br>
                    //                 Twarit Yojana - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_CUS('${wardNo}','cus','TWARIT YOJANA'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS Twarit Yojana');" style="color: #1cdad1;">${responseData.twarit}</a><br>
                    //                 Ghaziabad Smart City - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_CUS('${wardNo}','cus','Ghaziabad Smart City'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS Ghaziabad Smart City');" style="color: #f4ee40;">${responseData.Ghaziabadsmartcity}</a><br>
                    //                 Nagar Panchayat Nidhi - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_CUS('${wardNo}','cus','NAGAR PANCHAYT NIDHI'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS Nagar Panchayat Nidhi');" style="color: #bad057;">${responseData.panchayat}</a><br>
                    //                 UP Bridge Corporation Ltd - <a href="javascript:void(0)" onclick="Ghaziabad_Zone_CUS('${zoneNo}','cus','Up Bridge Corporation Ltd'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS UP Bridge Corporation Ltd');" style="color: #45d1a5;">${data.count_bridge}</a><br>

                    //                 NA - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_CUS('${wardNo}','cus','NA'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS NA');" style="color: #8717e2;;">${responseData.na}</a><br>
                    ...
                    //                 `  },
                    {
                        title: 'Type Sub Category',
                        value: `
                                Local Street - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_TypeSub('${wardNo}','category','Local Street'); updateNavBarWithFunctionName('Ward-${wardNo} Road Category Local Street');" style="color: #14cee3;">${responseData.count_local_street} </a><br>
                                Collector - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_TypeSub('${wardNo}','category','Collector'); updateNavBarWithFunctionName('Ward-${wardNo} Road Category Collector');" style="color: #e63dee;">${responseData.count_collector}</a><br>
                                Expressway - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_TypeSub('${wardNo}','category','Expressway'); updateNavBarWithFunctionName('Ward-${wardNo} Road Category Expressway');" style="color: #83e45c;">${responseData.count_expressway}</a><br>
                                Arterial - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_TypeSub('${wardNo}','category','Arterial'); updateNavBarWithFunctionName('Ward-${wardNo} Road Category Arterial');" style="color: #e1ca4c;">${responseData.count_arterial}</a><br>
                                Sub Arterial - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_TypeSub('${wardNo}','category','Subarterial'); updateNavBarWithFunctionName('Ward-${wardNo} Road Category Subarterial');" style="color: #83e45c;">${responseData.count_subarterial}</a><br>
                                NA - <a href="javascript:void(0)" onclick="Ghaziabad_Ward_TypeSub('${wardNo}','category','NA'); updateNavBarWithFunctionName('Ward-${wardNo} Road Category NA');" style="color: #ec1248;">${responseData.count_expressway}</a><br>
...
map.on('singleclick', function (evt) {
    ...
    // :white_check_mark: Case 1: WMS (has getFeatureInfoUrl)
    ...
    // :white_check_mark: Case 2: Vector layer (WFS GeoJSON)
    else {
        let found = false;
        map.forEachFeatureAtPixel(evt.pixel, function (feature, layer) {
            if (layer === currentLayer) {
                let props = feature.getProperties();
                // :white_check_mark: INSERT HERE
...
function highlightFeatureOnMap(gisId) {
    let wfsUrl = `${GEOSERVER_BASE_URL}/wfs?service=WFS&version=1.1.0&request=GetFeature
        &typename=GZB_Summary:ghaziabad_road
        &outputFormat=application/json
...
function ghaziabad_road_Length_Count() {
    removeCurrentLayer();
    clearVectorLayers();
    ...
    currentLayer = new ol.layer.Image({
        ...
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road',
...
function Ghaziabad_Ownership(cqlFilter) {
    removeCurrentLayer(); clearVectorLayers();
    currentLayer = new ol.layer.Image({
        //   title: 'Ward Boundary',
        //     extent: [-180, -90, -180, 90],
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_own',                                  // 'GZB_Summary:ayodhaya_road_ownership',
...
function Ghaziabad_Material(cqlFilter) {
    removeCurrentLayer(); clearVectorLayers();
    currentLayer = new ol.layer.Image({
        //   title: 'Ward Boundary',
        //     extent: [-180, -90, -180, 90],
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_mat',                                  //'GZB_Summary:ayodhaya_road_material',
                'CQL_FILTER': cqlFilter,
            },
            ratio: 1,
            serverType: 'geoserver'
...
function Ghaziabad_Types(cqlFilter) {
    removeCurrentLayer(); clearVectorLayers();
    currentLayer = new ol.layer.Image({
        //   title: 'Ward Boundary',
        //     extent: [-180, -90, -180, 90],
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_category',                                                        //'GZB_Summary:ayodhaya_road_data',
                'CQL_FILTER': cqlFilter,
            },
            ratio: 1,
            serverType: 'geoserver'
...
//             url: `${GEOSERVER_BASE_URL}/wms`,
//             params: {
//                 'LAYERS': 'GZB_Summary:z2ann_slum_boundary',
...
//             url: `${GEOSERVER_BASE_URL}/wms`,
//             params: {
//                 'LAYERS': 'GZB_Summary:z2ann_slum_roads',
...
$(document).ready(function () {
    ...
    var wfsParams = {
        ...
        typeName: 'GZB_Summary:ghaziabad_road_net',   // Replace with your WFS layer typename
    ...
    var vectorLayer = new ol.layer.Vector({
        ...
        style: new ol.style.Style({
            stroke: new ol.style.Stroke({
                ...
                width: 3 // Custom line width in pixels
            })
        })
    });
    map.addLayer(vectorLayer);
    // Define the URL for the WFS request
    const url = `${GEOSERVER_BASE_URL}/wfs?service=WFS&version=1.0.0&request=GetFeature&typeName=GZB_Summary:ghaziabad_road_net&outputFormat=application/json`;
...
const totalZones = 5;  // Number of wards you want to show
// Create options dynamically
...
function updateMapLayer(zoneNo) {
    ...
    currentLayer = new ol.layer.Tile({
        source: new ol.source.TileWMS({
            ...
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road',   // Replace with your actual layer name
                'TILED': true,
...
function updateMapLayertype(category) {
    ...
    const cqlFilter = `category='${category}'`; // Ensure the type is enclosed in quotes
    console.log(`Applying CQL_FILTER: ${cqlFilter}`);

    // Add a new WMS layer with the CQL_FILTER
    currentLayer = new ol.layer.Tile({
        source: new ol.source.TileWMS({
            title: `Category ${category} Roads`,
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_category',   // Replace with your actual layer name
                'TILED': true,
...
function updateMapLayermaterial(material) {
    ...
    currentLayer = new ol.layer.Tile({
        source: new ol.source.TileWMS({
            ...
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_mat',   // Replace with your actual layer name
                'TILED': true,
                'CQL_FILTER': cqlFilter // Apply single filter
            },
            ratio: 1,
...
function updateMapLayerownership(ownership) {
    ...
    currentLayer = new ol.layer.Tile({
        source: new ol.source.TileWMS({
            ...
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_own',   // Replace with your actual layer name
                'TILED': true,
                'CQL_FILTER': cqlFilter // Apply single filter
            },
            ratio: 1,
...
function updateMapAndTable() {
    ...
    if (selectedType) cqlFilter += ` AND category='${selectedType}'`;
...
function updateTableWithFilteredData(cqlFilter) {
    const dataTableBody_summaryfilter = document.getElementById('dataBody_summaryfilter');
    const wfsUrl = `${GEOSERVER_BASE_URL}/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=GZB_Summary:ghaziabad_road_category&outputFormat=application/json&CQL_FILTER=${encodeURIComponent(cqlFilter)}`;
...
const totalZonesmat = 5;  // Number of wards you want to show
// Create options dynamically
...
function updateMapLayertypemat(category) {
    ...
    currentLayer = new ol.layer.Tile({
        source: new ol.source.TileWMS({
            ...
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_mat',                               //  'GZB_Summary:ayodhaya_road_material', // Replace with your actual layer name
                'TILED': true,
                'CQL_FILTER': cqlFilter // Apply single filter
            },
            ratio: 1,
...
function updateMapAndTablemat() {
    ...
    if (selectedTypemat) cqlFilter += ` AND category='${selectedTypemat}'`;
...
function updateTableWithFilteredDatamat(cqlFilter) {
    ...
    const wfsUrl = `${GEOSERVER_BASE_URL}/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=GZB_Summary:ghaziabad_road_mat&outputFormat=application/json&CQL_FILTER=${encodeURIComponent(cqlFilter)}`;
...
function updateMapLayerOwn(zoneNo) {
    ...
    currentLayer = new ol.layer.Tile({
        source: new ol.source.TileWMS({
            ...
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_own',                                        //'GZB_Summary:ayodhaya_road_ownership', // Replace with your actual layer name
                'TILED': true,
                'CQL_FILTER': `zone_no=${zoneNo}` // Add dynamic filtering by zone_no
            },
            ratio: 1,
...
function updateMapAndTableOwn() {
    ...
    if (selectedTypeOwn) cqlFilter += ` AND category='${selectedTypeOwn}'`;
...
function updateTableWithFilteredDataOwn(cqlFilter) {
    ...
    const wfsUrl = `${GEOSERVER_BASE_URL}/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=GZB_Summary:ghaziabad_road_own&outputFormat=application/json&CQL_FILTER=${encodeURIComponent(cqlFilter)}`;
...
//     let layersMap = {
//         'type': 'GZB_Summary:ayodhaya_road_data',
//         'condition': 'GZB_Summary:ayodhaya_road_condition',
//         'material': 'GZB_Summary:ayodhaya_road_material',
//         'ownership': 'GZB_Summary:ayodhaya_road_ownership'
//     };
...
function Ghaziabad_Zone_no(column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road',   // Same layer for all filters
    ...
    fetchGNN_ALLFilteredData(column, value);
}

    function Ghaziabad_Ward_no(zone_no, column, value) {
    removeCurrentLayer();
    //  clearVectorLayers();

    let validColumns = ["ward_no","condition", "category", "material", "ownership", "cus"];
    ...
    if (["ward_no","material", "condition", "category", "ownership", "cus"].includes(column)) {
    ...
    fetchGNNFilteredData(zone_no, column, value);
}
//     removeCurrentLayer();
//     // clearVectorLayers();
...
//     fetchGNN_ALLFilteredData(column, value);  // Optional: You can enhance this function too if needed
// }

/*function Ghaziabad_Type_cat(column, value) {
    removeCurrentLayer();
  //  clearVectorLayers();
    // Define dynamic CQL filter
    let cqlFilter = `(${column}='${value}' OR ${column} ILIKE '%/${value}' OR ${column} ILIKE '${value}/%' OR ${column} ILIKE '%/${value}/%' OR ${column} ILIKE '${value}')`;
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_type',   // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ghaziabad_Condition_cat(column, value) {
    removeCurrentLayer();
    ///  clearVectorLayers();
    // Define dynamic CQL filter
    let cqlFilter = `(${column}='${value}' OR ${column} ILIKE '%/${value}' OR ${column} ILIKE '${value}/%' OR ${column} ILIKE '%/${value}/%' OR ${column} ILIKE '${value}')`;
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_condition',   // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ghaziabad_Material_cat(column, value) {
    removeCurrentLayer();
    //  clearVectorLayers();
    // Define dynamic CQL filter
    let cqlFilter = `(${column}='${value}' OR ${column} ILIKE '%/${value}' OR ${column} ILIKE '${value}/%' OR ${column} ILIKE '%/${value}/%' OR ${column} ILIKE '${value}')`;
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_mat',  // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ghaziabad_Ownership_cat(column, value) {
    removeCurrentLayer();
    //  clearVectorLayers();
    // Define dynamic CQL filter
    let cqlFilter = `(${column}='${value}' OR ${column} ILIKE '%/${value}' OR ${column} ILIKE '${value}/%' OR ${column} ILIKE '%/${value}/%' OR ${column} ILIKE '${value}')`;
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_own',   // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ghaziabad_CUS_cat(column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
    // Define dynamic CQL filter
    let cqlFilter = `(${column}='${value}' OR ${column} ILIKE '%/${value}' OR ${column} ILIKE '${value}/%' OR ${column} ILIKE '%/${value}/%' OR ${column} ILIKE '${value}')`;
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_cus',   // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ghaziabad_TypeSub_cat(column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
    // Define dynamic CQL filter
    let cqlFilter = `(${column}='${value}' OR ${column} ILIKE '%/${value}' OR ${column} ILIKE '${value}/%' OR ${column} ILIKE '%/${value}/%' OR ${column} ILIKE '${value}')`;
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_category',   // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function fetchGNN_ALLFilteredData(column, value) {
...
//--------------------Zonewise new code---------------------------------//
function Ghaziabad_Zone_Condition(zone_no, column, value) {
    removeCurrentLayer();
    //  clearVectorLayers();
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_condition',
                'CQL_FILTER': cqlFilter,
                'TILED': true
            },
            ratio: 1,
...
/*function Ghaziabad_Zone_Type(zone_no, column, value) {
    removeCurrentLayer();
  //  clearVectorLayers(); 
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_type',   // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ghaziabad_Zone_Material(zone_no, column, value) {
    removeCurrentLayer();
    //  clearVectorLayers();
    // Ensure column is valid to prevent errors
    let validColumns = ["condition", "category", "material", "ownership", "cus"];
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_mat',   // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ghaziabad_Zone_Ownership(zone_no, column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
    // Ensure column is valid to prevent errors
    let validColumns = ["condition", "category", "material", "ownership", "cus"];
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_own',   // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ghaziabad_Zone_CUS(zone_no, column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
    // Ensure column is valid to prevent errors
    let validColumns = ["condition", "category", "material", "ownership", "cus"];
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_cus',   // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ghaziabad_Zone_TypeSub(zone_no, column, value) {
    removeCurrentLayer();
    clearVectorLayers();
    // Ensure column is valid to prevent errors
    let validColumns = ["condition", "category", "material", "ownership", "cus",];
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_category',   // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function fetchGNNFilteredData(zone_no, column, value) {
...
/*function Ghaziabad_Ward_Type(ward_no, column, value) {
    removeCurrentLayer();
   // clearVectorLayers();
    ...
    // Fetch corresponding data
    fetchGNNWardFilteredData(ward_no, column, value);
}*/
function Ghaziabad_Ward_Condition(ward_no, column, value) {
    removeCurrentLayer();
    //  clearVectorLayers();
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GZB_Summary:ghaziabad_road_condition', // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ghaziabad_Ward_Material(ward_no, column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
    // Ensure column is valid to prevent errors
    let validColumns = ["condition", "category", "material", "ownership", "cus"];
...
function Ghaziabad_Ward_Ownership(ward_no, column, value) {
    removeCurrentLayer();
    //  clearVectorLayers();
...
function Ghaziabad_Ward_CUS(ward_no, column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
...
function Ghaziabad_Ward_TypeSub(ward_no, column, value) {
    removeCurrentLayer();
    clearVectorLayers();
    // Ensure column is valid to prevent errors
    let validColumns = ["condition", "category", "material", "ownership", "cus"];
...
// Function to fetch data dynamically based on Zone, Column, and Value
function fetchGNNWardFilteredData(ward_no, column, value) {
...
const downloadMapandExcel = document.querySelector(".fa-download");
